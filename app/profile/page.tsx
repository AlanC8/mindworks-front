...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        if (token) {
          const response = await axios.get("https://mindworks-back-production.up.railway.app/api/v1/auth/me", {
            headers: {
  ...
  const handleSave = async () => {
    try {
      const token = localStorage.getItem("access");
      if (token) {
        const response = await axios.put("https://mindworks-back-production.up.railway.app/api/v1/auth/update", userData, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
